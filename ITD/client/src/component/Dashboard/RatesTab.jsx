...
export default function RatesTab({evcpList}) {
  ...
  const handleSubmitSpecialOffer = async (e) => {
    e.preventDefault()
    setError('')
    const response = await fetch(`${BASE_API}/cpo/rate/special/${currentEvcp.evcpID}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({
      ...
      <div className="flex-col w-full mx-8">
        {specialOffer && specialOffer ? <>
          <div className='bg-white border-4 border-dash-red rounded-xl w-full p-4 flex-col max-h-min mb-4'>
            <p className="text-lg font-semibold text-center">Special Offer</p>
            <p className="text-md font-semibold text-center italic">{specialOffer.discount}% OFF</p>
          </div>
        </> : <></>}
                      ...
                      ? 
                        <>
                          ...
                          <td className='px-6 py-4'>
                            <span className="line-through mr-4">{rate.variablePrice}</span>
                            <span>{rate.variablePrice - rate.variablePrice * (specialOffer.discount) / 100}</span>
                          </td>
                          <td className='px-6 py-4'>
                            <span className="line-through mr-4">{rate.flatPrice}</span>
                            <span>{rate.flatPrice - rate.flatPrice * (specialOffer.discount) / 100}</span>
                          </td>
        ...
        <div className="flex justify-between gap-4">
          <div className="w-full">
            ...
            ? 
              <>
                <form id="addRate" className='bg-white p-4 w-full rounded-xl mt-4' onSubmit={handleSubmit}>
            ...
            {specialOffer && specialOffer
              ...
              <>
                {addSpecialOfferOpen && addSpecialOfferOpen
                  ?
                  <>
                    <form id="addSpecialOffer" className='bg-white p-4 w-full rounded-xl mt-4' onSubmit={handleSubmitSpecialOffer}>
                      <FormField
                        id="discount"
                        type="number"
                        value={discount}
