...
export default function tabSelector({tabs, onTabChange}) {
    ...
    useEffect(() => {
        ...
        document.getElementById(`text-${currentTab}`).classList.add('md:dark:text-tertiary', 'max-sm:font-semibold')
    }, [currentTab])
            ...
            <div className="flex md:flex-col w-full h-min justify-evenly md:justify-start md:bg-searchInput max-sm:border-b-2 max-sm:border-searchInput md:rounded-2xl md:p-4 md:mt-12 md:mx-6">
                {tabs.map(tab => (
                    ...
                    onClick={() => handleClick(tab)}>
                        ...
                        <p id={`text-${tab}`} className="max-sm:dark:text-tertiary text-right text-md md:font-semibold">{tab}</p>
                    </div>
                ))}
            </div>
        </div>
